...
{
    $user_data = Auth::user();
    $branchId = $request->branchId ?? $user_data->branchId;

    // Admin IDs (super users)
    $admin_ids = [1,23];
    ...
    if ($request->filled("date_range")) {
        $date_range = date_range_to_arr($request->date_range);
    } elseif ($request->has("date_range") && !$request->filled("date_range")) {
        $date_range = null;
    } else {
        $date_range = [date('Y-m-d', strtotime('-30 days')), date('Y-m-d')];
    }
    ...
    // 3️⃣ Branch filtering
    if (!$canViewAllBranches) {
        $purchases->where('branchId', $user_data->branchId);
        $check_p->where('branchId', $user_data->branchId);
    } elseif ($request->filled('branch_filter') && $isSuperAdmin) {
        $branch_filter = $request->branch_filter;
        $purchases->where('branchId', $branch_filter);
        $check_p->where('branchId', $branch_filter);
    }
    // Otherwise, branch-less or super admin sees all branches
    ...
    if ($hasSearch) {
        if ($request->filled('search_text')) {
            ...
            $purchases->where(function ($query) use ($search_text) {
                $query->where('dmr_no', '=', $search_text)
                    ...
                    ->orWhereHas('supplier', fn($q) => $q->where('name', 'like', '%' . $search_text . '%'))
                    ->orWhereHas('product_name', fn($q) => $q->where('name', 'like', '%' . $search_text . '%'))
                    ->orWhereHas('branch', fn($q) => $q->where('name', 'like', '%' . $search_text . '%'));
            });

            $check_p->where(function ($query) use ($search_text) {
                $query->where('dmr_no', '=', $search_text)
                    ->orWhere('chalan_no', '=', $search_text)
                    ->orWhere('product_qty', '=', $search_text)
                    ->orWhere('rate_per_unit', '=', $search_text)
        ...
        if ($request->filled('search_text2')) {
            ...
            $purchases->whereHas('product_name', fn($q) => $q->where('name', 'like', '%' . $search_text2 . '%'));
            $check_p->whereHas('product_name', fn($q) => $q->where('name', 'like', '%' . $search_text2 . '%'));
        }
    }

    // 5️⃣ Date range filtering
    if (isset($date_range)) {
        $purchases->whereBetween('received_date', [
            Carbon::parse($date_range[0])->format('Y-m-d'),
            Carbon::parse($date_range[1])->format('Y-m-d')
        ]);

        $check_p->whereBetween('received_date', [
            Carbon::parse($date_range[0])->format('Y-m-d'),
            Carbon::parse($date_range[1])->format('Y-m-d')
        ]);
    }
    ...
    // 9️⃣ Branch-wise totals
    $branch_totals_query = ProductPurchase::where('check_status', 0);

    if (!$canViewAllBranches) {
        $branch_totals_query->where('branchId', $user_data->branchId);
    } elseif ($request->filled('branch_filter') && $isSuperAdmin) {
        $branch_totals_query->where('branchId', $request->branch_filter);
    }

    if (isset($date_range)) {
        $branch_totals_query->whereBetween('received_date', [
            Carbon::parse($date_range[0])->format('Y-m-d'),
            Carbon::parse($date_range[1])->format('Y-m-d')
        ]);
    }
